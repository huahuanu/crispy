    ...
    def train(self, train_data, val_data, criterion, optimizer, checkpoint_filepath, writer, args):
        ...
        for epoch in range(epochs):
            ...
            for i, batch in enumerate(tqdm(train_data, desc=dis_str)):
                ...
                #     if hasattr(v,'bias') and isinstance(v.bias,nn.Parameter): #检查变量v中是否具有Bias属型，并且Bias是nn.Parameter的实例
                #         pg2.append(v.bias) #biases加入到参数组pg2中
                ...
                # if args.adam:
                #     optimizer = optim.Adam(pg0, lr=0.001, betas=(0.937,0.999))  # adjust beta1 to momentum
                
                # else:
                #     optimizer = optim.SGD(pg0, lr=0.001, momentum=0.937,nesterov=True) 
                
                # optimizer.add_param_group({'params':pg1,'weight_decay':0.0005}) # add pg1 with weight_decay
                # optimizer.add_param_group({'params':pg2}) # add pg2 (biases)
                # logger = logging.getLogger(__name__)
                # logger.info(f"{type(optimizer).__name__} with parameter groups "
                # f"{len(pg0)} weight, {len(pg1)} weight (no decay), {len(pg2)} bias")
                # del pg0,pg1,pg2

                # if args.linear_lr:  # 线性学习率调度器
                #     lf = lambda x: (1-x/(epochs-1)) * (1.0 - 0.1) + 0.1  # 0.1 final OneCycleLR learning rate (lr0 * lrf)
                # else:  #余弦退火学习率调度器
