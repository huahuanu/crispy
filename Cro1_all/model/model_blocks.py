...
class Fusion_MMTM(nn.Module):
    def __init__(self, args):
        ...
        self.mmtm = NEW_MMTM2(args.d_model, args.d_model, 4)
        self.layer_norm = nn.LayerNorm(args.d_model)
...
class NEW_MMTM2(nn.Module):
    def __init__(self, dim_bbox, dim_vel, ratio):
        super(NEW_MMTM2, self).__init__()
        dim = dim_bbox + dim_vel
        dim_out = int(2 * dim / ratio)
        self.MLP = nn.Linear(dim_out, dim_out)   # C->C    
        self.encoding = nn.Sequential(nn.Linear(dim, int(dim/2)),    # Linear的bias参数默认是True
                                  nn.ReLU(),
