...
class ModelInterface():
    ...
    def train(self, train_data, val_data, optimizer, checkpoint_filepath, writer, args):
        # cross = 0
        # not_cross = 0
        ...
        for epoch in range(epochs):
            ...
            for i, batch in enumerate(tqdm(train_data, desc=dis_str)):
                ...
                # print(f"cross:{cross},not_cross:{not_cross}")
                ...
                current_numbers += (predicted == label.squeeze()).sum().item()
                all_numbers += len(predicted)

            train_acc = current_numbers/all_numbers

            writer.add_scalar('train_loss', losses / (i + 1), epoch)
            writer.add_scalar('train_acc',train_acc,epoch)
            print(f"Epoch {epoch + 1}: Train loss: {losses / (i + 1)},Train acc:{train_acc }")

            valid_loss,valid_acc = self.evaluate(val_data, args)
            writer.add_scalar('val_loss', valid_loss, epoch)
            writer.add_scalar('val_acc', valid_acc,epoch)

            if best_score < valid_acc and args.monitor_acc:
                best_score = valid_acc
                num_steps_wo_improvement = 0
                torch.save(self.model.state_dict(),checkpoint_filepath)
                save_times += 1
                print("Save model {} times.\n".format(save_times))
            elif best_score > valid_loss and not args.monitor_acc:
    ...
    def evaluate(self, data, args):
        ...
        with torch.no_grad():
            ...
            val_acc = current_numbers/all_numbers

        print(f'Validation loss: {losses / (i + 1)}, Validation acc: {val_acc}')
        return losses / (i + 1),val_acc
